...
<!--      <el-table-column prop="courseid" label="课程id"></el-table-column>-->
      ...
      <el-table-column label="作业">
        <template v-slot="scope">
          <el-button type="primary" @click="viewPaper(scope.row.id)">查看作业</el-button>
        </template>
      </el-table-column>
      <el-table-column label="操作"  width="380" align="center">
        <template slot-scope="scope">
          <el-button type="primary" plain @click="handlePaper(scope.row.id, scope.row.courseid)">添加题目 <i class="el-icon-document"></i></el-button>
...
export default {
  ...
  methods: {
    ...
    handlePaper(jobId, courseId) {
      ...
      this.form2 = {handleQuestionIds: [], jobId: jobId}
