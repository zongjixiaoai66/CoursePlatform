...
<!--    </div>-->
    ...
    <div style="margin: 10px 0">
      ...
      <!-- <el-upload action="http://localhost:9999/teacherEvaluate/import" :show-file-list="false" accept="xlsx" :on-success="handleExcelImportSuccess" style="display: inline-block">
...
<!--      <el-table-column type="selection" width="55"></el-table-column>-->
      ...
      <el-table-column prop="grade" label="是否满意"></el-table-column>
    ...
    <el-dialog title="信息" :visible.sync="dialogFormVisible" width="30%" :close-on-click-modal="false">
      <el-form label-width="100px" size="small" style="width: 90%">
        <el-form-item label="教师名称">
          <el-select clearable v-model="form.teachername" placeholder="请选择" style="width: 100%">
        ...
        <el-form-item label="是否满意">
          <el-select clearable v-model="form.grade" placeholder="请选择" style="width: 100%">
...
export default {
  ...
  methods: {
    load() {
      this.request.get("/teacherEvaluate/page", {
        params: {
      ...
      this.request.get("/user/findTeacherNameList").then(res => {
    ...
    save() {
      ...
      this.request.post("/teacherEvaluate", this.form).then(res => {
        if (res.code === '200') {
    ...
    del(id) {
      this.request.delete("/teacherEvaluate/" + id).then(res => {
        if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.request.post("/teacherEvaluate/del/batch", ids).then(res => {
        if (res.code === '200') {
    ...
    exp() {
      window.open("http://localhost:9999/teacherEvaluate/export")
    },
